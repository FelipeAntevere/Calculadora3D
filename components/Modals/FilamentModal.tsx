            ...
            <div className="relative bg-white w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-8 space-y-5">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-900 mb-2 uppercase tracking-tight">Marca</label>
                            <input
                                type="text"
                                placeholder="Ex: Voolt3D"
                                value={filament.brand || ''}
                                onChange={(e) => setFilament({ ...filament, brand: e.target.value })}
                                className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#0ea5e9]/10 outline-none font-medium placeholder:text-slate-400"
                            />
                        </div>
                        <div className="relative">
                            <label className="block text-xs font-bold text-slate-900 mb-2 uppercase tracking-tight">Material</label>
                            <div className="relative">
                                <button
                                    onClick={() => setIsMaterialDropdownOpen(!isMaterialDropdownOpen)}
                                    className={`w-full flex items-center justify-between bg-[#f8fafc] border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none font-medium ${!filament.material ? 'text-slate-400' : 'text-slate-900'}`}
                                >
                                    {filament.material || 'Selecione...'}
                                    <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
                                </button>
                                {isMaterialDropdownOpen && (
                                    <div className="absolute left-0 mt-2 w-full bg-white border border-slate-200 rounded-xl shadow-2xl z-[60] py-2 max-h-[200px] overflow-y-auto animate-in fade-in zoom-in-95 origin-top">
                                        {materialOptions.map((mat) => (
                                            <button
                                                key={mat}
                                                onClick={() => { setFilament({ ...filament, material: mat }); setIsMaterialDropdownOpen(false); }}
                                                className="w-full flex items-center px-4 py-2 text-sm text-slate-600 hover:bg-slate-50 transition-colors"
                                            >
                                                <span>{mat}</span>
                                            </button>
                                        ))}
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-900 mb-2 uppercase tracking-tight">Cor</label>
                        <input
                            type="text"
                            placeholder="Ex: Azul Naval"
                            value={filament.color || ''}
                            onChange={(e) => setFilament({ ...filament, color: e.target.value })}
                            className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none font-medium placeholder:text-slate-400 focus:ring-2 focus:ring-[#0ea5e9]/10"
                        />
                    </div>

                    {!editingFilamentId && (
                        <div>
                            <label className="block text-xs font-bold text-slate-900 mb-2 uppercase tracking-tight">Quantidade de Carretéis</label>
                            <input
                                type="number"
                                min="1"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-900 mb-2 uppercase tracking-tight">Peso Inicial (kg)</label>
                            <input
                                type="number"
                                step="0.01"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-slate-900 mb-2 uppercase tracking-tight">Custo/kg (R$)</label>
                            <input
                                type="number"
                                step="0.01"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full bg-[#f8fafc] border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none font-medium text-slate-600 focus:ring-2 focus:ring-[#0ea5e9]/10"
                        />
                    </div>
                </div>
