            ...
            <div className="relative bg-white dark:bg-slate-800 w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <form onSubmit={(e) => { e.preventDefault(); onSave(); }}>
                    <div className="p-8 space-y-5">
                        <div>
                            <label className="block text-xs font-bold text-slate-900 dark:text-slate-300 mb-2 uppercase tracking-tight">Nome do Componente</label>
                            <input
                                type="text"
                                placeholder="Ex: Bico 0.4 Hardened Steel"
                                value={part.name || ''}
                                onChange={(e) => setPart({ ...part, name: e.target.value })}
                                className="w-full bg-[#f8fafc] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#0ea5e9]/10 dark:focus:ring-[#0ea5e9]/20 outline-none font-medium text-slate-700 dark:text-white placeholder:text-slate-400"
                                required
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-900 dark:text-slate-300 mb-2 uppercase tracking-tight">Categoria</label>
                                <select
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-900 dark:text-slate-300 mb-2 uppercase tracking-tight">Quantidade</label>
                                <input
                                    type="number"
                                    min="0"
                                    value={part.quantity || ''}
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-900 dark:text-slate-300 mb-2 uppercase tracking-tight">Marca (Opcional)</label>
                            <input
                                type="text"
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-900 dark:text-slate-300 mb-2 uppercase tracking-tight">Observações</label>
                            <textarea
                                rows={2}
                                placeholder="Notas sobre compatibilidade ou link da compra..."
                                value={part.notes || ''}
                                onChange={(e) => setPart({ ...part, notes: e.target.value })}
                                className="w-full bg-[#f8fafc] dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-[#0ea5e9]/10 dark:focus:ring-[#0ea5e9]/20 outline-none font-medium text-slate-700 dark:text-white placeholder:text-slate-400 resize-none"
                            />
                        </div>
                    </div>
                    ...
                    <div className="px-8 pb-8 flex items-center justify-end gap-3 bg-slate-50/50 dark:bg-slate-900/50 pt-4 border-t border-slate-50 dark:border-slate-700">
                        ...
                        <button
                            type="submit"
                            className="px-7 py-2.5 bg-[#0ea5e9] text-white rounded-xl text-xs font-bold hover:bg-[#0284c7] shadow-lg shadow-sky-100 dark:shadow-sky-900/20 transition-all active:scale-95"
                        >
